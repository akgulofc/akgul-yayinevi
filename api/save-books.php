...
if (!$ADMIN_SECRET || !$secret || $secret !== $ADMIN_SECRET) { http_response_code(401); echo json_encode(['error' => 'Yetkisiz erişim']); exit; }
if (!$GITHUB_TOKEN) { http_response_code(500); echo json_encode(['error' => 'GITHUB_TOKEN tanımlı değil']); exit; }
...
$books   = $body['books'];
$owner   = 'akgulofc';
$apiUrl  = "https://api.github.com/repos/{$owner}/akgul-yayinevi/contents/data/books.json";
$headers = [
